 ...
 */
public class Field2dDemacia implements NTSendable, AutoCloseable {
  /** Constructor. */
  ...
  @Override
  public void close() {
    for (FieldObject2dDemacia obj : m_objects) {
      obj.close();
    }
  }
   ...
   * @return Field object
   */
  public synchronized FieldObject2dDemacia getObject(String name) {
    for (FieldObject2dDemacia obj : m_objects) {
      if (obj.m_name.equals(name)) {
        return obj;
      }
    }
    FieldObject2dDemacia obj = new FieldObject2dDemacia(name);
    m_objects.add(obj);
    if (m_table != null) {
      synchronized (obj) {
   ...
   */
  public synchronized FieldObject2dDemacia getRobotObject() {
  ...
  private final List<FieldObject2dDemacia> m_objects = new ArrayList<>();
}